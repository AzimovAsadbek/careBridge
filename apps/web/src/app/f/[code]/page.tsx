    ...
    <div className="min-h-dvh bg-canvas">
      <header className="border-b border-line bg-white">
        <div className="mx-auto flex max-w-md items-center gap-3 px-4 py-3">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-xs text-slate-500">{q.header}</p>
            <h1 className="truncate text-[15px] font-semibold text-slate-900">{facility?.name ?? 'CareBridge'}</h1>
          </div>
          <LanguageSwitcher />
        </div>
      </header>
      ...
      <main className="mx-auto max-w-md px-4 pb-10 pt-6">
        {loadError ? (
          ...
          <div className="animate-enter rounded-[var(--radius-card)] border border-line bg-white p-8 text-center shadow-[var(--shadow-card)]" role="status">
          ...
          <form onSubmit={submit} className="space-y-6">
            <div>
              <h2 className="text-[22px] font-semibold tracking-tight text-slate-900">{q.title}</h2>
              <p className="mt-1 text-sm text-slate-600">{q.intro}</p>
              <ul className="mt-4 grid gap-2 rounded-[var(--radius-card)] border border-emerald-200 bg-emerald-50/60 p-3 text-[13px] text-emerald-950">
                {q.trust.map((line, i) => (
                  <li key={line} className="flex items-start gap-2">
                    <Icon name={(['lock', 'shield', 'clock'] as const)[i] ?? 'check'} className="mt-0.5 h-3.5 w-3.5 text-emerald-700" />
                    {line}
                  </li>
                ))}
              </ul>
              ...
              <div className="flex gap-2" role="radiogroup" aria-label={q.ratingGroup} onKeyDown={onRatingKey}>
                {[1, 2, 3, 4, 5].map((n) => (
                  <button
                    ...
                    onClick={() => setRating(n)}
                    className={cx(
                      'h-14 min-w-0 flex-1 rounded-[var(--radius-control)] border text-2xl transition-[color,background-color,transform] active:scale-95',
              ...
              <div className="grid grid-cols-2 gap-2">
                {TYPES.map((ty) => (
                  <button
                    ...
                    onClick={() => setType(ty)}
                    className={cx(
                      'h-12 rounded-[var(--radius-control)] border px-2 text-sm transition-colors',
                      type === ty ? 'border-brand-600 bg-brand-50 font-medium text-brand-800 ring-1 ring-inset ring-brand-600' : 'border-slate-300 bg-white text-slate-700 hover:bg-slate-50',
                    )}
                  >
            ...
            {/* Thumb-reach submit: pinned to the bottom of the screen on phones. */}
            <div className="sticky bottom-0 -mx-4 border-t border-line bg-canvas/95 px-4 pb-[max(0.75rem,env(safe-area-inset-bottom))] pt-3 backdrop-blur sm:static sm:mx-0 sm:border-0 sm:bg-transparent sm:p-0">
